...
class RTFM_loss(nn.Module):
    ...
    def forward(self, nscore, ascore, nlabel, alabel, nfeat, afeat):
        ...
        score = torch.cat((nscore, ascore), dim=0).squeeze()
...
def train(nloader, aloader, model, batch_size, optimizer):
    with torch.set_grad_enabled(True):
        ...
        nlabel = nlabel[:config.BATCH_SIZE]
        alabel = alabel[:config.BATCH_SIZE]
        input = torch.cat((ninput, ainput), dim=0).to(config.DEVICE)

        score_abnormal, score_normal, feat_select_abn, feat_sel_norm, feat_select_abn, feat_select_abn, x, feat_select_abn, feat_select_abn, feat_mag = model(input)
        
        scores = x.view(config.BATCH_SIZE * 32 * 2, -1).squeeze()
...
if __name__ == "__main__":
    ...
    nloader = DataLoader(ndata, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=config.NUM_WORKERS, pin_memory=False, drop_last=True)
    aloader = DataLoader(adata, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=config.NUM_WORKERS, pin_memory=False, drop_last=True)
